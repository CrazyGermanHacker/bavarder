...
def index(users):
    ...
    extend_([u'    <div style="text-align: right;"><i class="material-icons" style="font-size: 32px;">more_vert</i></div>\n'])
    extend_([u'</div>\n'])
    extend_([u'\n'])
    ...
    for x in loop.setup(range(len(users))):
        extend_(['        ', u'<div class="chatbutton" onclick="chat_action(\'', escape_(users[x], True), u'\')">\n'])
